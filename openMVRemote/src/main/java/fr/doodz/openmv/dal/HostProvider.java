...
public class HostProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        if (!values.containsKey(Hosts.NAME)) {
            Resources r = Resources.getSystem();
            values.put(Hosts.NAME, r.getString(android.R.string.untitled));
        }

        if (!values.containsKey(Hosts.ADDR)) {
            values.put(Hosts.ADDR, "");
        }
        if (!values.containsKey(Hosts.PORT)) {
            values.put(Hosts.PORT, 0);
        }
        if (!values.containsKey(Hosts.USER)) {
            values.put(Hosts.USER, "");
        }
        if (!values.containsKey(Hosts.PASS)) {
            values.put(Hosts.PASS, "");
        }
        if (!values.containsKey(Hosts.ESPORT)) {
            values.put(Hosts.ESPORT, 0);
        }
        if (!values.containsKey(Hosts.TIMEOUT)) {
            values.put(Hosts.TIMEOUT, -1);
        }
        if (!values.containsKey(Hosts.WIFI_ONLY)) {
            values.put(Hosts.WIFI_ONLY, 0);
        }
        if (!values.containsKey(Hosts.ACCESS_POINT)) {
            values.put(Hosts.ACCESS_POINT, "");
        }
        if (!values.containsKey(Hosts.MAC_ADDR)) {
            values.put(Hosts.MAC_ADDR, "");
        }
        if (!values.containsKey(Hosts.WOL_PORT)) {
            values.put(Hosts.WOL_PORT, 0);
        }
        if (!values.containsKey(Hosts.WOL_WAIT)) {
